...
namespace HTMLConverter
{
    ...
    public static class HtmlToXamlConverter
    {
        ...
        private static XmlNode AddBlock(XmlElement xamlParentElement, XmlNode htmlNode, Hashtable inheritedProperties, CssStylesheet stylesheet, List<XmlElement> sourceContext)
        {
            ...
            else if (htmlNode is XmlElement)
            {
                ...
                switch (htmlElementName)
                {
                    ...
                    case "ol":
                    case "ul":
                    case "sortDirection": //  treat as UL element
                ...
                Helpers.Assert(sourceContext.Count > 0 && sourceContext[sourceContext.Count - 1] == htmlElement);
        ...
        /// <summary>
        /// Converts Html ul or ol element into Xaml item element. During conversion if the ul/ol element has any children 
        /// that are not li elements, they are ignored and not added to the item element
        /// </summary>
        /// <param name="xamlParentElement">
        /// XmlElement representing Xaml parent to which the converted element should be added
        /// </param>
        ...
        private static void AddList(XmlElement xamlParentElement, XmlElement htmlListElement, Hashtable inheritedProperties, CssStylesheet stylesheet, List<XmlElement> sourceContext)
        {
            ...
            {
                // Unordered item - all elements other than OL treated as unordered lists
                xamlListElement.SetAttribute(HtmlToXamlConverter.Xaml_List_MarkerStyle, Xaml_List_MarkerStyle_Disc);
            }

            // Apply local properties to item to set marker attribute if specified
            // TODO: Should we have separate item attribute processing function?
            ApplyLocalProperties(xamlListElement, localProperties, /*isBlock:*/true);

            // Recurse into item subtree
            for (XmlNode htmlChildNode = htmlListElement.FirstChild; htmlChildNode != null; htmlChildNode = htmlChildNode.NextSibling)
            {
                if (htmlChildNode is XmlElement && htmlChildNode.LocalName.ToLower() == "li")
                {
                    ...
                    Helpers.Assert(sourceContext.Count > 0 && sourceContext[sourceContext.Count - 1] == htmlChildNode);
                    sourceContext.RemoveAt(sourceContext.Count - 1);
                }
                else
                {
        ...
        /// <summary>
        /// If li topics are found without a parent ul/ol element in Html string, creates xamlListElement as their parent and adds
        /// them to it. If the previously added node to the same xamlParentElement was a List, adds the elements to that item.
        ...
        /// </summary>
        /// <param name="xamlParentElement">
        /// Parent element for the item
        /// </param>
        /// <param name="htmlLIElement">
        /// Start Html li element without parent item
        /// </param>
        /// <param name="inheritedProperties">
        /// Properties inherited from parent context
        /// </param>
        ...
        {
            Helpers.Assert(htmlLIElement.LocalName.ToLower() == "li");
            ...
            {
                // Previously added Xaml element was a item. We will add the new li to it
                xamlListElement = (XmlElement)xamlListItemElementPreviousSibling;
            }
            else
            {
                // No item element near. Create our own.
        ...
        private static void AddListItem(XmlElement xamlListElement, XmlElement htmlLIElement, Hashtable inheritedProperties, CssStylesheet stylesheet, List<XmlElement> sourceContext)
        {
            ...
            Helpers.Assert(xamlListElement != null);
            Helpers.Assert(xamlListElement.LocalName == Xaml_List);
            Helpers.Assert(htmlLIElement != null);
            Helpers.Assert(htmlLIElement.LocalName.ToLower() == "li");
            Helpers.Assert(inheritedProperties != null);
        ...
        {
            // Parameter validation
            Helpers.Assert(htmlTableElement.LocalName.ToLower() == "table");
            Helpers.Assert(xamlParentElement != null);
            Helpers.Assert(inheritedProperties != null);
            ...
            if (singleCell != null)
            {
                ...
                Helpers.Assert(sourceContext.Count > 0 && sourceContext[sourceContext.Count - 1] == singleCell);
                sourceContext.RemoveAt(sourceContext.Count - 1);
            }
            else
            {
        ...
        private static void AddColumnInformation(XmlElement htmlTableElement, XmlElement xamlTableElement, ArrayList columnStartsAllRows, Hashtable currentProperties, CssStylesheet stylesheet, List<XmlElement> sourceContext)
        {
            ...
            {
                #region GEMCOM (adds support for Table.Columns element)
                XmlElement xamlColumnsElement = xamlTableElement.OwnerDocument.CreateElement(null, Xaml_TableColumns, _xamlNamespace);
                ...
                for (int columnIndex = 0; columnIndex < columnStartsAllRows.Count - 1; columnIndex++)
                {
                    ...
                    #region GEMCOM (adds support for Table.Columns element)
                    //xamlTableElement.AppendChild(xamlColumnElement); uncomment this if !GEMCOM
        ...
        {
            // Parameter validation
            Helpers.Assert(xamlTableBodyElement.LocalName == Xaml_TableRowGroup);
            Helpers.Assert(currentProperties != null);
            ...
            while (htmlChildNode != null && htmlChildNode.LocalName.ToLower() != "tbody")
            {
                ...
                else if (htmlChildNode.LocalName.ToLower() == "td")
                {
                    ...
                    Helpers.Assert(columnStarts == null);
        ...
        {
            // parameter validation
            Helpers.Assert(xamlTableRowElement.LocalName == Xaml_TableRow);
            Helpers.Assert(currentProperties != null);
            if (columnStarts != null)
            {
                Helpers.Assert(activeRowSpans.Count == columnStarts.Count);
            }
            ...
            while (htmlChildNode != null && htmlChildNode.LocalName.ToLower() != "tr" && htmlChildNode.LocalName.ToLower() != "tbody" && htmlChildNode.LocalName.ToLower() != "thead" && htmlChildNode.LocalName.ToLower() != "tfoot")
            {
                if (htmlChildNode.LocalName.ToLower() == "td" || htmlChildNode.LocalName.ToLower() == "th")
                {
                    ...
                    if (columnStarts != null)
                    {
                        Helpers.Assert(columnIndex < columnStarts.Count - 1);
                        while (columnIndex < activeRowSpans.Count && (int)activeRowSpans[columnIndex] > 0)
                        {
                            ...
                            Helpers.Assert((int)activeRowSpans[columnIndex] >= 0);
                            columnIndex++;
                        }
                        Helpers.Assert(columnIndex < columnStarts.Count - 1);
                        ...
                        Helpers.Assert(columnSpan > 0);
                        Helpers.Assert(columnIndex + columnSpan < columnStarts.Count);
                        ...
                        {
                            Helpers.Assert(spannedColumnIndex < activeRowSpans.Count);
                            activeRowSpans[spannedColumnIndex] = (rowSpan - 1);
                            Helpers.Assert((int)activeRowSpans[spannedColumnIndex] >= 0);
                        }
        ...
        {
            // Parameter validation
            Helpers.Assert(xamlTableCellElement.LocalName == Xaml_TableCell);
            Helpers.Assert(currentProperties != null);
        ...
        private static ArrayList AnalyzeTableStructure(XmlElement htmlTableElement, CssStylesheet stylesheet)
        {
            ...
            Helpers.Assert(columnStarts.Count == activeRowSpans.Count);
        ...
        /// attributes is stored in the reference ArrayList parameter columnStarts, which contains a item of all starting
        ...
        {
            // Parameter validation
            Helpers.Assert(htmlTbodyElement.LocalName.ToLower() == "tbody");
            Helpers.Assert(columnStarts != null);
        ...
        private static double AnalyzeTRStructure(XmlElement htmlTRElement, ArrayList columnStarts, ArrayList activeRowSpans, double tableWidth, CssStylesheet stylesheet)
        {
            ...
            // Parameter validation
            Helpers.Assert(htmlTRElement.LocalName.ToLower() == "tr");
            Helpers.Assert(columnStarts != null);
            Helpers.Assert(activeRowSpans != null);
            Helpers.Assert(columnStarts.Count == activeRowSpans.Count);
            ...
            {
                Helpers.Assert((double)columnStarts[columnIndex] >= columnStart);
            ...
            while (htmlChildNode != null && columnWidthsAvailable)
            {
                ...
                {
                    case "td":
                        Helpers.Assert(columnIndex <= columnStarts.Count);
                        if (columnIndex < columnStarts.Count)
                        {
                            Helpers.Assert(columnStart <= (double)columnStarts[columnIndex]);
                        ...
                        if (columnWidth != -1)
                        {
                            ...
                            if (nextColumnIndex != -1)
                            {
                                ...
                                Helpers.Assert(nextColumnIndex <= columnStarts.Count);
        ...
        /// </summary>
        /// <param name="columnStarts">
        /// Array item representing starting coordinates of all columns in the table
        /// </param>
        ...
        private static int GetNextColumnIndex(int columnIndex, double columnWidth, ArrayList columnStarts, ArrayList activeRowSpans)
        {
            ...
            // Parameter validation
            Helpers.Assert(columnStarts != null);
            Helpers.Assert(0 <= columnIndex && columnIndex <= columnStarts.Count);
            Helpers.Assert(columnWidth > 0);
        ...
        private static double GetNextColumnStart(XmlElement htmlTDElement, double columnStart)
        {
            ...
            // Parameter validation
            Helpers.Assert(htmlTDElement.LocalName.ToLower() == "td" || htmlTDElement.LocalName.ToLower() == "th");
            Helpers.Assert(columnStart >= 0);
        ...
        private static int CalculateColumnSpan(int columnIndex, double columnWidth, ArrayList columnStarts)
        {
            ...
            Helpers.Assert(columnStarts != null);
            Helpers.Assert(columnIndex < columnStarts.Count - 1);
            Helpers.Assert((double)columnStarts[columnIndex] >= 0);
            Helpers.Assert(columnWidth > 0);
            ...
            while (columnSpanningValue <  columnWidth && columnSpanningIndex < columnStarts.Count - 1)
            {
                ...
                Helpers.Assert(subColumnWidth > 0);
        ...
        private static void ApplyLocalProperties(XmlElement xamlElement, Hashtable localProperties, bool isBlock)
        {
            ...
            while (propertyEnumerator.MoveNext())
            {
                ...
                {
                    case "font-family":
                        //  Convert from font-family value item into xaml FontFamily value
        ...
        private static Hashtable GetElementProperties(XmlElement htmlElement, Hashtable inheritedProperties, out Hashtable localProperties, CssStylesheet stylesheet, List<XmlElement> sourceContext)
        {
            ...
            switch (elementName)
            {
                ...
                // List properties
                case "ul":
                    localProperties["item-style-type"] = "disc";
                    break;
                case "ol":
                    localProperties["item-style-type"] = "decimal";
                    break;
        ...
        {
            // Parameter validation
            Helpers.Assert(htmlChildNode.LocalName.ToLower() == "td" || htmlChildNode.LocalName.ToLower() == "th");
            Helpers.Assert(xamlTableCellElement.LocalName == Xaml_TableCell);
