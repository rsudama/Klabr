    ...
    internal static class HtmlFromXamlConverter
    {
        ...
        private static bool WriteFlowDocument(XmlTextReader xamlReader, XmlTextWriter htmlWriter)
        {
            ...
            if (xamlReader.NodeType != XmlNodeType.Element || 
                (xamlReader.Name != "FlowDocument" && xamlReader.Name != "Section"))
            {
                // Root FlowDocument or Section element is missing
                return false;
            }
        ...
        {
            Helpers.Assert(xamlReader.NodeType == XmlNodeType.Element);
        ...
        private static void WriteElementContent(XmlTextReader xamlReader, XmlTextWriter htmlWriter, StringBuilder inlineStyle)
        {
            ...
            else
            {
                while (ReadNextToken(xamlReader) && xamlReader.NodeType != XmlNodeType.EndElement)
                {
                    switch (xamlReader.NodeType)
                    {
                        case XmlNodeType.Element:
                            ...
                            Helpers.Assert(xamlReader.NodeType == XmlNodeType.EndElement || xamlReader.NodeType == XmlNodeType.Element && xamlReader.IsEmptyElement);
                            break;
                ...
                Helpers.Assert(xamlReader.NodeType == XmlNodeType.EndElement);
            }
        }
        ...
        /// <summary>
        /// Reads several topics from xamlReader skipping all non-significant stuff.
        /// </summary>
        /// <param name="xamlReader">
		...
		private static bool ReadNextToken(XmlReader xamlReader)
		{
			...
			{
				Helpers.Assert(xamlReader.ReadState == ReadState.Interactive, "Reader is expected to be in Interactive state (" + xamlReader.ReadState + ")");
				switch (xamlReader.NodeType)
				{
				    case XmlNodeType.Element: 
